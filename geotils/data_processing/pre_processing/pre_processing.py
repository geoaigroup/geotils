    ...
    def pre_load(
        ...
        for file in os.listdir(self.image_directory):
            ...
            if filename.endswith(self.image_suffix):
                ...
                mask = rasterio.open(
                    glob.glob(f"{mask_directory}/{name}{mask_suffix}")[0]
                )
                mask = mask.read()[0]
                full_img = cv2.resize(
                    full_img,
                    (WIDTH_orig // DOWN_SAMPLING, HEIGHT_orig // DOWN_SAMPLING),
                )

                full_img, rrp_info = ratio_resize_pad(
                    full_img, ratio=None, div=fragment_size
                )
                full_mask, mask_rrp_info = ratio_resize_pad(
                    mask, ratio=None, div=fragment_size
                )
