...
class CropGeoTiff:
            ...
            with rasterio.open(input_path) as src:
                ...
                for row in range(num_rows):
                    for col in range(num_cols):
                        ...
                        window = Window(
                            col * self.fragment_size[1],
                            row * self.fragment_size[0],
                        ...
                        fragment_profile["transform"] = rasterio.windows.transform(
                        ...
                        fragment_output_path = os.path.join(
                            save_subdir, f"{name}_{ind}.tif"
                        )
                        with rasterio.open(
                            fragment_output_path, "w", **fragment_profile
                        ) as dst:
                            dst.write(fragment)
                        ind += 1
    ...
    def crop_shapefiles(self, images, shapefile_path, output_dir):
        ...
        @param images = path of the diectory containing cropped images
            i.e each image should have a seaprate folder with all of its cropped fragments
    ...
    def crop_nongeoref_images_shapefile(
        self, input_dir, shapefile_path, output_image_dir, output_shp_dir
    ):
        """
        @param input_dir = path of the directory containing images to be cropped
        @param shapefile_path = path of the directory containing shapefiles to be cropped
