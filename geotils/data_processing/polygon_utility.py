    ...
    if transform:
        for orig_row in geo:
            ...
            if orig_row.geom_type == "Polygon":
                binary_array = geometry_mask(
                    [orig_row], out_shape=shape, transform=transform, invert=True
                )
                ba = binary_array * 1
                gtmask = gtmask + ba
            else:
                for x in orig_row.geoms:
                    binary_array = geometry_mask(
                        [x], out_shape=shape, transform=transform, invert=True
                    )
                    ba = binary_array * 1
                    gtmask = gtmask + ba
    else:
    ...
    if transform:
        for row in geo:
            if row.geom_type == "Polygon":
                binary_array = geometry_mask(
                    [row], out_shape=shape, transform=transform, invert=True
                )
                ba = binary_array * 1
                gtmask.append(ba)
...
def shape_polys(polyg: List[Polygon]) -> List[List[Tuple[float, float]]]:
    ...
    for poly in polyg:
        if len(poly) >= 3:
            f = poly.reshape(-1, 2)
            simplified_vw = simplify_coords_vwp(f, 0.3)
...
def pol_to_bounding_box(pol: List[List[float]]) -> BoundingBox:
    ...
    arr = pol_to_np(pol)
    return BoundingBox(
        np.min(arr[:, 0]), np.min(arr[:, 1]), np.max(arr[:, 0]), np.max(arr[:, 1])
    )
