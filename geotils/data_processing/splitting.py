...
class Splitting:
    ...
    Args:
        ...
        label_keys (list): List of keys for labels. Only needed when verbose is true
        verbose (bool, default=False): If True, print fold statistics.
    """

    def __init__(
        self, df, nfolds, seed, id_key, split_key, label_keys=None, verbose=False
    ):
        self.df = df
        self.nfolds = nfolds
        self.seed = seed
        ...
        if self.verbose:
            ...
            for fold in folds:
                for label_key in self.label_keys:
                    ...
                    print(
                        self.df.loc[self.df["fold"] == fold]
                        .set_index(["fold", label_key])
                        .groupby(level=label_key)
                        .count()
                    )
